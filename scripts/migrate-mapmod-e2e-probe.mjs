...
for (const file of ['tests/e2e/desktop-map-request.spec.js', 'tests/e2e/desktop-map-overlay-request.spec.js', 'tests/e2e/desktop-browser-global-surface.spec.js', dashboardPath]) {
  if (fs.readFileSync(file, 'utf8').includes('MapMod')) throw new Error(`${file} still contains MapMod`);
}
