...
| `js/main-ui.js` | Primary route-search orchestration, route-result DOM wiring, map/list UI and in-app page navigation; theme/list/modal/navigation implementations remain closure-local, with cross-file page changes entering through `navigation:request` |
| `js/enhancements.js` | Cross-cutting interaction enhancements; requests page changes through the navigation event boundary |
...
| `js/ride-tools.js` | Rider utility features; requests page changes through the navigation event boundary |
...
| `js/desktop-dashboard.js` | Desktop command-center orchestration; elevation/playback remains closure-local, route cameras come from `route:updated`, vehicle mode/plate come from `vehicle:changed`, and page changes are requested through `navigation:request` |
...
7. **Shared globals — incremental cleanup in progress.** The first audit removed internal-only `ThemeMod`, `ListMod`, `ModalMod`, and `DesktopElevationMod` from `window`. The next pass has removed three categories of direct coupling: `desktop-dashboard.js` no longer reads `RouteMod.filteredCams`, `RouteMod.mode`, or `RouteMod.plate`; camera snapshots arrive through `route:updated`, vehicle state arrives through `vehicle:changed`, and the desktop module owns local `state.routeCameras` plus `state.vehicle` copies. PWA offline restoration calls `RouteMod.setVehicle()` so it participates in the same vehicle event path. Page-navigation consumers in `desktop-dashboard.js`, `enhancements.js`, and `ride-tools.js` now emit `navigation:request` instead of calling `NavMod.go()` directly. A production-source audit found no remaining `NavMod` consumers outside `main-ui.js`, so `window.NavMod` was removed and the implementation is closure-local. Continue this pattern only where an existing event or narrowly scoped capability provides a clearer dependency direction.
...
The focused desktop refactor suite covers layout keyboard resizing, ARIA state, persistence across reloads, bounds and reset behavior, MapLibre route source/fitted state, clickable CCTV markers, synthetic condition rendering, route-condition runtime delegation, route-search endpoint-preparation delegation, route-summary DOM copy delegation, external Google/Apple navigation target decisions, the supported browser-global surface, desktop route-camera state consumption from the `route:updated` payload, desktop vehicle state consumption from the `vehicle:changed` payload, and page switching through `navigation:request` without a global `NavMod`. Static checks prevent internal-only `ThemeMod`, `ListMod`, `ModalMod`, `NavMod`, and `DesktopElevationMod` from being re-exported to `window`; prevent `desktop-dashboard.js` from directly reading `RouteMod.filteredCams`, `RouteMod.mode`, or `RouteMod.plate`; require PWA offline vehicle restoration to use `RouteMod.setVehicle()`; and require `desktop-dashboard.js`, `enhancements.js`, and `ride-tools.js` to use the navigation event boundary instead of referencing `NavMod`.

GitHub Actions CI #152 passed on the navigation-event-boundary head: static/unit checks and the focused desktop Chromium regression were both green.
