...
(function() {
  ...
  function weatherFeature(section) {
    ...
    return { type: 'Feature', properties: {}, geometry: { type: 'Point', coordinates: toLngLat(middle) } };
  }
  ...
  function addEventMarker(renderer, incident, section) {
    ...
    var safeTitle = typeof window.escapeHtml === 'function' ? window.escapeHtml(incident.title || kind) : String(incident.title || kind);
    element.innerHTML = '<span class="desktop-event-icon">' + (kind === 'construction' ? '⚠' : kind === 'accident' ? '!' : kind === 'weather' ? '☁' : '•') + '</span><span class="desktop-event-callout">' + safeTitle + '</span>';
  ...
  function install(renderer) {
    ...
    renderer.drawConditionSections = function(sections) {
      ...
      sectionFeatures.forEach(function(feature) {
        coords = coords.concat(feature.geometry.coordinates.map(function(point) { return [point[1], point[0]]; }));
      });
  ...
  if (window.MapRenderer && typeof window.MapRenderer.create === 'function' && !window.MapRenderer.__conditionLayerInstalled) {
