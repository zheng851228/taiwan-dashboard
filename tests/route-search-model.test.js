...
describe('RouteSearchModel', () => {
  ...
  it('keeps unmatched cached route points out of the resolution plan', async () => {
    const model = await loadModel();
    const plan = model.buildAddressPlan({
      startValue: '台北車站',
      endValue: '高雄車站',
      startRoutePoint: '25.0478,121.5170',
      startRoutePointLabel: '其他地點',
      endRoutePoint: '22.6396,120.3028',
      endRoutePointLabel: '高雄車站'
    });
    expect(Array.from(plan.displayAddrs)).toEqual(['台北車站', '高雄車站']);
    expect(Array.from(plan.resolutionAddrs)).toEqual(['台北車站', '22.6396,120.3028']);
  });
});
