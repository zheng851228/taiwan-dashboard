...
test.describe('desktop layout controller', () => {
  ...
  test('shift keyboard resizing uses the larger step and Home/End respect bounds', async ({ page }) => {
    await openReadyDesktop(page);
    ...
    const atMinimum = await readLayout(page);
    expect(Number(await handle.getAttribute('aria-valuenow'))).toBe(atMinimum.left);
    expect(atMinimum.left).toBe(Number(await handle.getAttribute('aria-valuemin')));

    await page.keyboard.press('End');
    const atMaximum = await readLayout(page);
    expect(Number(await handle.getAttribute('aria-valuenow'))).toBe(atMaximum.left);
    expect(atMaximum.left).toBe(Number(await handle.getAttribute('aria-valuemax')));
  });

  test('reset clears the persisted preference and restores defaults', async ({ page }) => {
    await openReadyDesktop(page);
    ...
    await expect.poll(() => page.evaluate(key => localStorage.getItem(key), LAYOUT_KEY)).toBeNull();
  });
});
